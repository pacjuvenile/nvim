...
local plugins_config_full_dir = vim.fn.glob(lua_dir .. '/custom/plugins/**/*.lua', false, true)
for _, plugin_config_full_dir in ipairs(plugins_config_full_dir) do
  ...
  local ok, plugin_config = pcall(require, plugin_config_module)
  -- 组单个插件配置规格表
  if ok and type(plugin_config) == "table" and plugin_config.ensure_installed ~= false then
